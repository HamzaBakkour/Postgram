...
class ResetPasswordConfirm(GenericAPIView, ListModelMixin):
    throttle_classes = ()
    permission_classes = ()
    ...
    def get(self, request, *args, **kwargs):
        ...
        logger.debug("request vars: {}".format(vars( request)))
        logger.debug("request.data : {}".format(request.data))
        token = request.GET.get('token', '')
        logger.debug("token: {}".format(token))

        return redirect("https://postgram.hamzabakkour.se/login/")
